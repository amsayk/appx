...
const messages = /*defineMessages*/({
  ...
  // },
  minimum: 'Doit être supérieur à {value}',

  // maximum: {
  //   id: 'error.maximum',
  //   defaultMessage: 'Doit être inférieur à {value}',
  // },
  maximum: 'Doit être inférieur à {value}',

  // oneOf: {
  //   id: 'error.oneOf',
  //   defaultMessage: 'Doit être l\'un des: '
  // },
  oneOf: 'Doit être l\'un des: ',
...
export function createValidator(rules) {
  return (data = {}) => {
    ...
    Object.keys(rules).forEach((key) => {
      ...
      const error = rule(getProp(data, key), data);
      if (error) {
        errors[key] = error;
      }
    });
